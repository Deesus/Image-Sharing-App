...
def login_required(routed_function):
    """A wrapper to check if user is logged prior to acccessing page.

    Args:
        routed_function: to function to be checked. An example of a
        routed_function is 'new_album'"""

    @wraps(routed_function)
    def decorator(*args, **kwargs):
        if ('username' not in login_session):
            return redirect(url_for('show_login', next=request.url))
        return routed_function(*args, **kwargs)
    return decorator
